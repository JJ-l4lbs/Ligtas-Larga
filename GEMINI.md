...
* **Relentless Verification Before Completion:** Always exhaustively verify all changes and project state before declaring any task complete. If browser-based verification tools or MCPs (e.g., Playwright) are available in the workspace environment, you must use them to run validation tests on user interface components, verify styling layout parameters, check browser log errors, and ensure functional correctness. Upon finishing the validation, ensure to cleanly close the browser session using `browser_close` to prevent orphaned execution processes.
